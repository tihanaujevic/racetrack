...
function aStar(startPoint, finishPoints, obstacles, grass) {
  let openSet = new PriorityQueue({
    comparator: (a, b) => {
      if (a.f === b.f) {
        return Math.random() - 0.5; // Random tiebreaking
      }
      return a.f - b.f;
    }
  });
  ...
  while (openSet.length > 0) {
    ...
    let neighbors = getPossibleMoves(currentPoint, currentVelocity, grass)
      .filter(([point, velocity]) =>
        !obstacleInPath(obstacles, currentPoint, point) &&
        !closedSet.has(stateToString(point, velocity))
      );

    neighbors = shuffleArray(neighbors); // Shuffle neighbors to introduce randomness
    ...
    for (const [neighbor, newVelocity] of neighbors) {
      ...
      if (neighborNode) {
        ...
        neighborNode = {
          ...
          parent: currentNode,
          index: Math.random() // Assign a random index
        };
        openSet.queue(neighborNode);
        openSetMap.set(neighborState, neighborNode);
      }
    ...
    for (let dy = -1; dy <= 1; dy++) {
      ...
      if (Number.isInteger(newPosition[0]) && Number.isInteger(newPosition[1])) {
        ...
        if (newPosition[0] >= 0 && newPosition[1] >= 0) {
          // Check if the new position is on grass
          const newPositionOnGrass = grass.some(g => g[0] === newPosition[0] && g[1] === newPosition[1]);
